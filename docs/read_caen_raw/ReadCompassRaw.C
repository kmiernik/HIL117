...
int main(int argc, char* argv[])
{
  ...
  TFile *fOutput = new TFile(Form("%s.root",argv[2]),"recreate");
  ...
  *sizeFrame = *sizeFrame & 0x0FFFFFFF;
  ...
  uint32_t *convBuff;
  // READING THE COMPASS HEADER 
  while(fread(buffer,1,sizeof(uint32_t),fInput)){
    convBuff = (uint32_t*)buffer;
    if((convBuff[0]>>24)==0xA0)break;
    else printf("Skipping %08x\n",convBuff[0]);
  }
  printf("current pos = %ull ",ftell(fInput));
  fseek(fInput,-4,SEEK_CUR);
  printf(" after shift = %ull \n",ftell(fInput));
  uint32_t aggregateSize =0;
  uint32_t nbAggr=0;
  uint32_t offset = 5;
  uint32_t channelAggrSize = 0;
  FILE *pFile;
  pFile = fopen(Form("%s.adf",argv[2]),"wb");
