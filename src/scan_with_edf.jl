...
"""
    event_builder!(chunk, last_event, spectra, specpars, t_event, edffile;
...
"""
function event_builder!(chunk, last_event, spectra, specpars, t_event, edffile;
                       valid_table, cal_table, fcal_table, shift_table,
                       type_table, distance_table, pid_table)
    d_target_neda = 1.0
    hits = [last_event;]
    ...
    for i in 2:n_hits
        if hits[i].t - hits[event[1]].t < t_event
            ...
            write(edffile, Header(UInt8(Mraw), UInt8(M), UInt8(ges),
            ...
            for j in 1:M
                ...
                write(edffile, EDFHit(UInt8(hits[event[j]].loc),
                                      Eedf, tedf, UInt8(hits[event[j]].pid)))
                jloc = hits[event[j]].loc
                if type_table[jloc] != 1
                    continue
                end
...
function scan_run(data_dir, config::Dict, prefix; 
    ...
    edfnameout = prefix * ".edf"
    edffile = open(edfnameout, "w")
    println("    \u25E6 EDF file ", edfnameout, " created")
